...
from run_lite_mode_comparison import float_value, parse_elapsed_seconds  # noqa: E402
from server.recommender import build_candidate_pool, detect_image_size  # noqa: E402
...
DEFAULT_OUTPUT_CSV = ROOT_DIR / "report" / "tables" / "persistent_worker_comparison.csv"
DEFAULT_LOG = ROOT_DIR / "report" / "logs" / "persistent_worker_comparison.txt"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare subprocess SimOPA scoring with the persistent SimOPA worker."
    )
...
def main() -> None:
    ...
    full_rows_by_case = group_rows(read_rows(Path(args.full_ranking_csv)), "case_id")
    full_summary_by_case = {row["case_id"]: row for row in read_rows(Path(args.full_summary_csv))}
    ...
    try:
        for index, case in enumerate(cases, start=1):
            ...
            print(f"[{index:02d}/{len(cases):02d}] worker scoring {case_id}")
    ...
    print(f"worker_candidate_rows={sum(int(row['worker_candidate_count']) for row in comparison_rows)}")
...
def compare_case(
    *,
    ...
    prior_candidates = build_candidate_pool(
        ...
        scorer_mode="simopa-worker",
    )
    worker_candidates = [annotation_candidate]
    for prior in prior_candidates:
        worker_candidates.append(
            {
                **prior,
                "rank": len(worker_candidates) + 1,
    ...
    worker_results = score_candidate_boxes(
    ...
    worker_case_elapsed_ms = round((time.perf_counter() - case_started) * 1000.0, 2)

    scored_worker = list(zip(worker_candidates, worker_results))
    scored_worker.sort(key=lambda item: item[1].score, reverse=True)

    full_sorted = sorted(full_rows, key=lambda row: int(row["model_rank"]))
    full_top3 = full_sorted[:3]
    worker_top3 = scored_worker[:3]
    worker_dataset_rank, worker_dataset_score = dataset_position_rank(scored_worker)

    full_top3_ids = [row["candidate_id"] for row in full_top3]
    worker_top3_ids = [candidate["candidate_id"] for candidate, _ in worker_top3]
    overlap_count = len(set(full_top3_ids) & set(worker_top3_ids))
    worker_assessment = expected_case_result(case["label"], worker_dataset_score, worker_dataset_rank)
    ...
    return {
        ...
        "full_mode": "simopa-subprocess",
        "worker_mode": "simopa-worker",
        "full_candidate_count": len(full_rows),
        "worker_candidate_count": len(worker_candidates),
        "full_top1_id": full_top3[0]["candidate_id"],
        "worker_top1_id": worker_top3[0][0]["candidate_id"],
        "top1_matches": "yes" if full_top3[0]["candidate_id"] == worker_top3[0][0]["candidate_id"] else "no",
        "full_top1_score": full_top3[0]["score"],
        "worker_top1_score": round(worker_top3[0][1].score, 4),
        "full_top3_ids": ";".join(full_top3_ids),
        "worker_top3_ids": ";".join(worker_top3_ids),
        ...
        "worker_dataset_position_rank": worker_dataset_rank,
        "full_dataset_position_score": full_summary["dataset_position_score"],
        "worker_dataset_position_score": round(worker_dataset_score, 4),
        "full_assessment": full_summary["assessment"],
        "worker_assessment": worker_assessment,
        "assessment_matches": "yes" if full_summary["assessment"] == worker_assessment else "no",
        "full_inner_runtime_ms": round(sum(float_value(row.get("runtime_ms")) for row in full_rows), 2),
        "worker_inner_runtime_ms": round(sum(result.runtime_ms for result in worker_results), 2),
        "worker_case_elapsed_ms": worker_case_elapsed_ms,
        "full_model_version": full_top3[0]["model_version"],
        "worker_model_version": worker_top3[0][1].model_version,
    }
...
def write_log(
    ...
    *,
    ...
    worker_score_calls = sum(int(row["worker_candidate_count"]) for row in rows)
    ...
    speedup = full_elapsed_seconds / worker_elapsed_seconds if full_elapsed_seconds and worker_elapsed_seconds else 0.0

    with path.open("w", encoding="utf-8") as handle:
        handle.write("SmartPlace SimOPA subprocess-vs-worker comparison\n")
        handle.write("model_version=simopa-worker-rgb-mask-v1\n")
        handle.write(f"cases={len(rows)}\n")
        handle.write(f"full_score_calls={full_score_calls}\n")
        handle.write(f"worker_score_calls={worker_score_calls}\n")
        handle.write(f"full_elapsed_seconds={full_elapsed_seconds:.1f}\n")
        handle.write(f"worker_elapsed_seconds={worker_elapsed_seconds:.1f}\n")
        ...
        for row in rows:
            handle.write(
                ...
                f"worker_assessment={row['worker_assessment']} "
                f"worker_elapsed_ms={row['worker_case_elapsed_ms']}\n"
            )
