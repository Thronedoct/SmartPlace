...
本文是 SmartPlace 模型相关工作的唯一权威文档，合并了课程参考源码审阅、模型路线决策、高分方案、数据集方案、本地训练可行性和手机端推理分析。后续模型路线变化优先改本文，避免多份文档互相漂移。
...
目标：确保 Android -> 后端 -> 真实模型 -> Top 3 展示的端到端链路可跑。
...
结论：第一版不建议把真实模型放到手机端推理。推荐主线是云端/电脑端 FastAPI 推理，Android 只做交互和展示。

原因：

- Android 端需要模型转换，例如 ONNX、TFLite 或 MindSpore Lite。
- `RGB + mask` 4 通道输入在移动端预处理更麻烦。
- 端侧推理要处理模型包体、内存、耗时和兼容性。
- 课程演示最重要的是稳定闭环；后端推理更容易展示日志和模型真实性。
...
如果后续一定要做端侧推理，建议作为加分项：

| 路线 | 可行性 | 说明 |
|---|---|---|
| ONNX Runtime Mobile | 中等 | PyTorch -> ONNX 较自然，但 Android 集成和 4 通道输入预处理要额外做。 |
| TFLite | 中等偏低 | PyTorch -> ONNX -> TensorFlow/TFLite 链路更长。 |
| MindSpore Lite | 中等偏低 | 如果课程强调华为生态可作为亮点，但迁移成本较高。 |
| 手机只做轻量规则候选，模型仍在后端 | 高 | 最稳，交互体验好，模型证据也清楚。 |

如果做端侧推理，只转换 V1/V2 小模型，不要转 TopNet 或 FOPAHeatMapModel。
...
| Android 展示模型字段 | 成员 C | Top 3、分数、标签、耗时、模型版本 |
