...
- LightOPA 轻量模型探索已完成：tiny 4 通道 CNN 在 2,000/500 OPA 子集上达到验证准确率 `0.65`、ROC-AUC `0.6761`；residual 4 通道 CNN 在 3,000/600 OPA 子集上达到验证准确率 `0.6717`、ROC-AUC `0.7084`。这两者作为轻量模型 baseline，不替代主线 `simopa-worker`。
- 运行耗时表和模型改动说明表已完成：`inference_runtime.csv` 汇总 15 个运行阶段，`model_change_summary.csv` 汇总 14 个已完成改动。
...
当前主线已经具备高分交付基础。后续方向不是再堆散实验，而是按“四段式”把项目卷到更完整、更好看、更容易交给队友接手的状态：
...
目标：在不破坏 `simopa-worker` 主线的前提下，再补一轮可讲、可量化、风险可控的工作。

优先做：

1. **LightOPA stronger baseline**：residual 版已经完成。若还继续卷，可以再尝试 ResNet18/MobileNetV3 级别轻量 scorer，继续使用 composite RGB + mask 输入。若依赖或训练质量不稳定，只保留为探索，不影响主线。
2. **Web 最终演示验收**：启动 `simopa-worker`，对 5 个内置案例跑完整流程，验证 Top 3、可信度提示、解释热力图入口、JSON/CSV 导出、演示模式、桌面和移动端布局。
3. **可选附录级候选生成对比**：只在依赖顺利时尝试 FOPA/TopNet 的候选生成对比，目标是附录级“看过并解释取舍”，不替换当前规则候选 + SimOPA 排序主线。
...
- 公网部署。
- 大规模 fine-tune。
- 全量测试集跑满，除非已有脚本能低成本复用。

### S2 前端重构级美化

目标：让 Web 从“能演示的工作台”升级成“看起来像完整课程作品的推理应用”。

当前进度：第一轮浅色工作台重构已完成，页面改为左侧输入/中间画布/右侧结果的三栏布局；`web/app.js` 已拆出 API、可信度分析、导出、图像预览和候选框渲染模块。

计划内容：

1. **结构重构**：已把 `web/app.js` 中的 API 调用、画布渲染、demo case、导出、可信度提示拆成更清晰的小模块；后续只做必要维护，不再回到单文件堆叠。
2. **视觉重构**：已切换为浅色工作台风格，不做 landing page；后续继续细化图像画布、Top 3 面板、模型状态 badge、候选框层级、按钮状态、空/加载/错误状态。
3. **演示模式强化**：让课堂演示时重点信息更醒目，包括 `scorer_mode`、`model_version`、`runtime_ms`、request id、Top 3 分数和可信度提示。
4. **响应式验收**：用桌面和移动视口检查无横向溢出、文字不挤压、候选框不遮挡关键信息；必要时补 Playwright 验证脚本或截图。

### S3 项目结构深度整理

目标：把项目从“不断推进的研发仓库”整理成“队友和老师一眼能读懂的交付仓库”。

计划内容：

1. **目录整理**：梳理 `experiments/`、`report/`、`assets/`、`server/`、`web/` 的职责边界；必要时补 `README.md` 或索引说明，但不新增重复路线文档。
...
目标：工程侧把材料整理到队友可以直接写报告、做 PPT、录屏的状态。
...
5. **风险说明**：写清楚哪些是主线已完成，哪些是探索项，哪些不做，避免答辩时被追问带偏。
