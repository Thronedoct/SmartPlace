...
1. **结构重构**：把 `web/app.js` 中的 API 调用、状态管理、画布渲染、demo case、导出、可信度提示拆成更清晰的小模块或局部函数区块，减少后续维护成本。
2. **视觉重构**：保持工作台风格，不做 landing page；优化三栏布局、图像画布、Top 3 面板、模型状态 badge、候选框层级、按钮状态、空/加载/错误状态。
