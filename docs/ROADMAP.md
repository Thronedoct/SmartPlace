...
1. **LightOPA stronger baseline**：在 tiny LightOPA 已完成的基础上，尝试 ResNet18/MobileNetV3 级别轻量 scorer，继续使用 composite RGB + mask 输入。产出训练日志、验证指标、推理耗时和与 tiny/SimOPA 的对比表。若依赖或训练质量不稳定，只保留为探索，不影响主线。
