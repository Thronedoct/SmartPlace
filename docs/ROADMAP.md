...
- 运行耗时表和模型改动说明表已完成：`inference_runtime.csv` 汇总 7 个运行阶段，`model_change_summary.csv` 汇总 8 个已完成改动和 3 个 planned_next 项。
- Web 工作台已支持内置案例加载、当前结果 JSON/CSV 导出、可信度/失败提示、解释热力图入口和基础易用性增强。
...
1. **前端美化与演示模式深化**：继续把 Web 工作台升级成更像完整产品的界面，优化桌面/移动布局、图像画布、候选框视觉层级、模型状态 badge、Top 3 面板、空状态、加载态和错误态。
2. **Web 证据联动增强**：把导出的 JSON/CSV、解释热力图、案例图和可信度提示组织成更清晰的课堂演示模式。
