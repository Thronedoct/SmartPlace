...
| `mode` | string | 否 | `auto`、`mock`、`simopa` 或 `simopa-lite`，默认 `auto`。`auto` 使用服务启动时的 `SMARTPLACE_SCORER`。 |
...
- 表单 `mode=mock`、`mode=simopa` 或 `mode=simopa-lite`：覆盖服务默认 scorer，便于测试。
...
返回 Web 工作台可一键加载的代表案例。案例来自 `report/tables/failure_cases.csv`、`report/tables/opa_18_case_summary.csv` 和本地 OPA 数据集，不会提交 raw 图片到 GitHub。
...
当本地未下载 OPA raw 数据或缺少截图时，对应资源返回 `404`，`available=false` 的案例按钮会在前端禁用。
